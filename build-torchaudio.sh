...
export TORCHAUDIO_REPO_LOC="/mnt/sharedfs/megatron/pytorch/torchaudio-repo"
export NVCC_GENCODE="-gencode=arch=compute_90,code=sm_90 -gencode=arch=compute_90a,code=sm_90a"
export TAG_OR_COMMIT="17a70815259222570feb071034acd7bae2adc019"
...
if [ ! -d "${TORCHAUDIO_REPO_LOC}" ]; then
  git clone https://github.com/pytorch/audio "${TORCHAUDIO_REPO_LOC}"
fi

# Enter pytorch repo
pushd "${TORCHAUDIO_REPO_LOC}" || exit
